...
To address the issue of lexical mismatch, \citep{Petrovic:2012:UPI:2382029.2382072} suggest a method of integrating paraphrasing with their LSH-based approach, which shows improvements when applied to TDT datasets, but fails to show any significant improvement on Twitter data (in fact, it often performs worse).
...
Using the graphical structure of Twitter, where each user is a node in a graph, their followers/followees form the vertices.
Each cluster contains two summaries: (i) a node-summary, which is a set of users and their frequencies and (ii) a content-summary, which is a set of words and their TF-IDF \citep{Salton:1988:TAA:54259.54260} weighted frequencies.
...
Indeed, nearest neighbour clustering alone seems inadequate as a method of tracking the discussion of complex or evolving events.
...
\todo{A page or two of content to come}
...
\todo{Discussion around some of the evaluation issues to come}
