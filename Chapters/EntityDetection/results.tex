...
Precision beats that of the LSH baseline significantly, despite being disadvantaged due to the partial relevance judgements.
...
Our approach also appears to effective at detecting events in the `Business \& Economy' ($R=0.391$), `Sports' ($R=0.373$), and `Law, Politics \& Scandals' ($R=0.386$) categories.
...
Although the performance differences between 6 and 24 periods is small, and there does not appear to be a single best history length across the range of event sizes, there is a clear and significant difference between limited and unlimited historical data.
...
Clusters are only added to an event if they contain more the minimum number of tweets, so the minimum cluster size has a significant effect on the overall performance, particularity at lower minimum event sizes.
...
Increasing the minimum cluster size has a significant effect on precision at both minimum event sizes.
